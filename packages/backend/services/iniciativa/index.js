...
const lists_file = fs.readFileSync(require.resolve(`../../../../forms/iniciativa/lists1.yml`), 'utf8');
...
class Service {
  ...
  async getDraftInfo(id) {
    ...
    const proponente = await getProponente('iniciativa', id);
    policy.organizacao = {
      id: proponente?.id,
    }
  ...
  async verify(id, form, indic_forms) {
    ...
    const connections = await require('../entity').verify('iniciativa',id);
  ...
  async getDraftIndic(form, indic_name, id) {
    ...
    for (let f of form.fields.filter(f => ['file', 'thumbnail'].includes(f.type))) {
      ...
      if (indicator?.[f.key] && !isNaN(indicator?.[f.key])) {
        ...
        indicator[f.key] = {
          url: `${process.env.S3_CONTENT_URL}/${this.getFileKey(id, 'iniciativa', f.key, fileModel.get('url'))}`,
  ...
  async saveDraftIndic(user, form, indic_name, entity, files, id) {
    const iniciativa = await db.models['Iniciativa'].findOne(
      { where: { politica_id: id, versao: 'draft' } },
      {
        raw: true,
        nest: true,
  ...
  async hasGeo(id) {
    ...
    from iniciativas.iniciativas pa
  ...
  async enterInInitiative(user) {
    /* descobre o id da comunidade rede de iniciativa */
    const result = await db.instance().query(
      `
      select id
      from dorothy_communities dc
      where alias = 'rede_iniciativas'
    `,
      {
        type: Sequelize.QueryTypes.SELECT,
      },
  ...
  async createInitiative(nome, user) {
    ...
    result = await db.instance().query(
      `
    SELECT MAX(politica_id)+1 as next from iniciativas.iniciativas
    `,
      {
        type: Sequelize.QueryTypes.SELECT,
      },
    ...
    result = await db.instance().query(
      `
    INSERT into iniciativas.iniciativas(nome, community_id, politica_id, versao, "createdAt", "updatedAt") values(:nome, :community_id, :politica_id, 'draft', NOW(), NOW())
    `,
      {
  ...
  async getGeoDrawSave(id, geoms) {
    ...
    await db.instance().query(
      `
        delete from iniciativas.iniciativas_atuacao where politica_versao_id = :politica_versao_id`,
      {
    ...
    for (let idx = 0; idx < geoms.length; idx++) {
      ...
      await db.instance().query(
        `
        insert into iniciativas.iniciativas_atuacao(politica_versao_id, geom)
    ...
    const ufs = await db.instance().query(
      ...
      with w_points as (
          ...
          WHEN ST_GeometryType(par.geom) = 'ST_Point' then ST_Buffer(par.geom, 0.00001, 'quad_segs=8')
              ...
              par.politica_versao_id
          from iniciativas.iniciativas_atuacao par
      ), u_geom as (
          select
      ...
      const [bbox] = await db.instance().query(
        `
            with bounds as (
              select ST_Extent(geom) as bbox
              from iniciativas.iniciativas_atuacao pa
              inner join iniciativas.iniciativas p on p.id = pa.politica_versao_id and p.versao = 'draft'
  ...
  async getInfoForParticipation(id) {
    ...
    const members = await sequelize.query(
      ...
      inner join iniciativas.iniciativas p on p.community_id = m."communityId"
  ...
  async saveDraftTimeline(user, entity, timeline_arquivo, id, tlid) {
    ...
    if (!tlid) {
      ...
      // recupera
      entityModel = await db.models['Iniciativa_timeline'].findByPk(tlid);
      // atualiza
  ...
  async removeDraftTimeline(id, tlId) {
    ...
    await db.models['Iniciativa_timeline'].destroy({
      where: {
        id: tlId,
        politica_versao_id,
      },
  ...
  async getIniciativaDraftId(politica_id) {
    // encontra a versao draft desta politica
    const p_draft = await db.instance().query(
      `
  ...
  async delete(id, user) {
    ...
    if (result) {
      ...
      await db.models['Iniciativa'].destroy({
        where: {
          politica_id,
        },
      });
  ...
  async publish(politica_id) {
    ...
    try {
      ...
      const politica_current = await db.models['Iniciativa'].findOne({
        where: {
          politica_id,
          versao: 'current',
        },
      ...
      if (politica_current) {
        const timelines = await db.models['Iniciativa_timeline'].findAll({
          where: {
        ...
        await db.instance().query(
          `
            DELETE FROM iniciativas.iniciativas_atuacao
            WHERE politica_versao_id = :politica_versao_id
          `,
          {
      ...
      /**** >>> B.DRAFT->CURRENT <<< ****/
      const politica_draft = await db.models['Iniciativa'].findOne({
        where: {
          politica_id,
          versao: 'draft',
        },
      ...
      // C.2 CLONE politica
      const politica_clone = await db.models['Iniciativa'].create(
        {
      ...
      for (let tl of timelines) {
        ...
        await db.models['Iniciativa_timeline'].create(
          {
            ...tl.get({ plain: true }),
            id: undefined,
      ...
      await db.instance().query(
        `
        INSERT INTO iniciativas.iniciativas_atuacao(politica_versao_id, geom)
        SELECT ${politica_clone.id} as politica_versao_id, geom FROM iniciativas.iniciativas_atuacao
        WHERE politica_versao_id = :politica_versao_id
      `,
        {
