...
(async function setupForms() {
  /* INFORMAÇÃO */
  const form1 = await FormManager.getForm('iniciativa/form1');
  ...
  const indic_forms = await FormManager.getForms('iniciativa/indics');
  let indic_forms_form = {};

  for (let form of indic_forms) {
    const indic_name = form.replace('indic_', '').replace('.yml', '');
    const indic_form = await FormManager.getForm(`iniciativa/indics/indic_${indic_name}`);
