module.exports.check = (oForm, data) => {
  ...
  const form = structuredClone(oForm) //{ ...oForm }

  // console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MEMORY (check)', Object.keys(form).length, Object.keys(form))

  // merge elements of blocks
  for (let b of form.blocks) {
    // console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MEMORY (check b)', b , structuredClone(b) )
    b.elements = mergeBlockElement(b, form)
    // console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MEMORY (check b)', { b })
  }
...
function mergeBlockElement(b, form) {
  ...
  // console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> MEMORY (mergeBlockElement)', b.elements?.length, { b_elements: b.elements })

  for (let f of form.fields.filter(f => f.block === b.key)) {
    elements.push(f.key)
  }
  elements.sort((a) => a.type === 'block' ? 1 : 0)
