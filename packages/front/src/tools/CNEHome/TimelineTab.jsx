...
export default function TimelineTab({ entityId }) {
  ...
  //get cne_data
  const { data } = useQuery(['cne_timeline', { entityId }], {
    queryFn: async () => (await axios.get(`${server}cne/${entityId}/draft/timeline`)).data,
    retry: false,
  ...
  const mutationRemove = useMutation(
    id => {
      return axios.delete(`${server}cne/${entityId}/draft/timeline/${id}`);
    },
    {
      onSuccess: () => queryClient.invalidateQueries(`cne_timeline`),
    },
  );
...
function TimelineManager({ add, entityId, editing, onHighlight, onCancel, onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if(add) { /* insert */
        method = 'post';
        url = `${server}cne/${entityId}/draft/timeline`;
      } else {
       /* edit */
        method = 'put';
        url = `${server}cne/${entityId}/draft/timeline/${editing.id}`;
      }
      ...
      queryClient.invalidateQueries('cne_timeline');
