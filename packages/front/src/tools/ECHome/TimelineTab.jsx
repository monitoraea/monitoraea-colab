...
export default function TimelineTab({ entityId }) {
  ...
  //get initiative_data
  const { data } = useQuery(['educom_clima_timeline', { entityId }], {
    queryFn: async () => (await axios.get(`${server}educom_clima/draft/timeline`)).data,
    retry: false,
  ...
  const mutationRemove = useMutation(
    id => {
      return axios.delete(`${server}educom_clima/draft/timeline/${id}`);
    },
    {
      onSuccess: () => queryClient.invalidateQueries(`educom_clima_timeline`),
    },
  );
...
function TimelineManager({ add, entityId, editing, onHighlight, onCancel, onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if(add) { /* insert */
        method = 'post';
        url = `${server}educom_clima/draft/timeline`;
      } else {
       /* edit */
        method = 'put';
        url = `${server}educom_clima/draft/timeline/${editing.id}`;
      }
      ...
      queryClient.invalidateQueries('educom_clima_timeline');
