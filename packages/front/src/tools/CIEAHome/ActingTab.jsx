...
async function retrieveGeoms(server, id) {
  const {
    data: { geoms, bbox },
  } = await axios.get(`${server}commission/${id}/geo-draw`);
...
export default function ActingTab() {
  ...
  useEffect(() => {
    async function fetchID() {
      const {
        data: { id },
      } = await axios.get(`${server}commission/id_from_community/${currentCommunity.id}`);
      _id(id);
    }
    fetchID();
  ...
  useEffect(() => {
    async function hasGeo() {
      ...
      } = await axios.get(`${server}commission/${id}/geo-draw/has-geo`);
  ...
  const handleSave = async geoms => {
    ...
    const { data } =  await axios.post(`${server}commission/${id}/geo-draw`, {
      geoms: geomsToSave,
    });
  ...
  const handleSaveUFs = async () => {
    ...
    await axios.put(`${server}commission/${id}/draft/ufs`, { ufs });
