...
export default function TimelineTab({ projectId }) {
  ...
  //get project_data
  const { data } = useQuery(['project_timeline', { projectId }], {
    queryFn: async () => (await axios.get(`${server}project/${projectId}/draft/timeline`)).data,
    retry: false,
  ...
  const mutationRemove = useMutation(
    id => {
      return axios.delete(`${server}project/${projectId}/draft/timeline/${id}`);
    },
    {
      onSuccess: () => queryClient.invalidateQueries(`project_timeline`),
    },
  );
                        ...
                        entity.map(e => (
                          <Fragment key={e.id}>
                            {editing?.id !== e.id && (
                              <div
                                id={`timeline_${e.id}`}
                                className={`${styles.each} ${toHighlight2 === e.id ? styles.highlight : ''}`}
                              >
                                <div className={styles.date}>
                                  {' '}
                                  <span className={styles.text}>
                                    {dayjs(e.date).locale('pt-br').format('MMM [de] YYYY')}
                                  </span>
                                </div>
                                ...
                                <div className={styles.buttons}>
                                  ...
                                  {editing?.id !== e.id && (
                                    <button className={styles.action} onClick={remove(e.id)} disabled={!!editing}>
                                      <Trash></Trash>
                                    </button>
                                  )}
                                </div>
...
function TimelineManager({ add, projectId, editing, onHighlight, onCancel, onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (add) {
        /* insert */
        method = 'post';
        url = `${server}project/${projectId}/draft/timeline`;
      } else {
        /* edit */
        method = 'put';
        url = `${server}project/${projectId}/draft/timeline/${editing.id}`;
      }
      ...
      queryClient.invalidateQueries('project_timeline');
  ...
  return (
    <div className={`row ${!add && !!editing ? styles.editing : ''}`}>
      ...
      <div className={`col-xs-1 ${styles['vcentered']}`}>
        {add && (
          <button className={styles.add} onClick={handleSave} disabled={add && !!editing}>
            <Plus></Plus>
          </button>
        )}
        {!add && (
