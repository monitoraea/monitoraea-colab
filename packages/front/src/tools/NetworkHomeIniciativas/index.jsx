...
export default function NetworkHomeCIEA() {
  ...
  const { data } = useQuery(
    ['user_iniciatives_list', { order, direction }],
    {
      queryFn: async () => (await axios.get(`${server}iniciativa/mine?direction=${direction}`)).data,
    }
  );
  ...
  const mutation = {
    create: useMutation(name => axios.post(`${server}iniciativa`, { nome: name }), {
        onSuccess: () => {
          queryClient.invalidateQueries(`user_iniciatives_list`)
        },
    }),
};
  ...
  const doParticipate = async (isADM, policyId) => {
    ...
    const { data } = await axios.post(
      `${server}iniciativa/${policyId}/participate`,
      {
        isADM,
      },
    );
  ...
  const handleNInitiative = async name => {
    ...
    try {
      ...
      _showNIDialog(false);
      window.location = `/colabora/iniciativa/${data.communityId}`; /* TODO: melhorar */
    } catch (error) {
      closeSnackbar(snackKey);
  ...
  return (
    <div>
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle id="alert-dialog-title">Nova iniciativa de não governamental</DialogTitle>
        <DialogContent>
          <div className="row">
