...
import { FloatFormat } from '../../../components/FormatedValues';
...
export default function Viz({ lae_id, indic_id, question_id, title, filters = '', totalAplica }) {
  ...
  const { data } = useQuery(`adm/statistics/indic/int2/${lae_id}/${indic_id}/${question_id}/?${filters}`);
  const { data: avg } = useQuery(`adm/statistics/indic/int1/${lae_id}/${indic_id}/${question_id}/?${filters}`);
