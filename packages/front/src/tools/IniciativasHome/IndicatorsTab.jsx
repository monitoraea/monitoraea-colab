...
export default function IndicatorsTab({ entityId, analysis, problems }) {
  ...
  //get policy_data
  const { data } = useQuery([`iniciative_info_${currentIndics}`, { entityId }], {
    queryFn: async () => (await axios.get(`${server}iniciativa/${entityId}/draft/indicadores/${currentIndics}`)).data,
  ...
  const { data: content } = useQuery([`content_for_form_${currentIndics}`], {
    queryFn: async () => (await axios.get(`${server}content/for_form/ing/${currentIndics}/indic`)).data,
    enabled: !!currentIndics,
    retry: false,
    refetchOnWindowFocus: false,
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      /* edit */
      method = 'put';
      url = `${server}iniciativa/${entityId}/draft/indicadores/${currentIndics}`;
      ...
      queryClient.invalidateQueries(`iniciative_info_${currentIndics}`);
              ...
              {!!currentForm && (
                <Renderer
                  ...
                  helpbox={{
                    ...
                    content,
                    prefix: `ing.${currentIndics.split('_')[0]}.${currentIndics}.`,
                  }}
                />
              )}
