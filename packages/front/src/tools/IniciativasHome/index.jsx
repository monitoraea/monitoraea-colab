...
const Manager = () => {
  ...
  //get entity_id
  const { data: entity } = useQuery(['iniciatives', { currentCommunity: currentCommunity.id }], {
    queryFn: async () => (await axios.get(`${server}iniciativa/id_from_community/${currentCommunity.id}`)).data,
    retry: false,
    refetchOnWindowFocus: false,
  });

  const { data: analysis } = useQuery(['iniciatives_analysis', { policy_id: entity?.id }], {
    queryFn: async () => (await axios.get(`${server}iniciativa/${entity?.id}/verify`)).data,
    enabled: !!entity?.id,
    retry: false,
    refetchOnWindowFocus: false,
  });
  ...
  const mutation = {
    publish: useMutation(() => axios.put(`${server}iniciativa/${entityId}/publish`)),
    remove: useMutation(() => {
      return axios.delete(`${server}iniciativa/${toRemove}`);
    }),
  };
  ...
  const handleDownload = () => {
    window.open(`${server}iniciativa/${entityId}/download`, '_blank');
  };

  return (
    <div className="page">
      <div className="page-header">
        <PageTitle title="Iniciativa Não Governamental de Educação Ambiental" />
        ...
        <>
          ...
          {entityId && (
            <>
              ...
              {tabindex === 'conexoes' && <ConexoesTab entityName="iniciativa" entityId={entityId} onSave={()=>queryClient.invalidateQueries('iniciatives_analysis')} />}
