...
import form from '../../../../../forms/iniciativa/form1.yml';
import form_view from '../../../../../forms/iniciativa/form1_view.yml';
import lists from '../../../../../forms/iniciativa/lists1.yml';
...
export default function InformationsTab({ entityId, problems }) {
  ...
  const { data: content } = useQuery([`content_for_form_cad`], {
    queryFn: async () => (await axios.get(`${server}content/for_form/ing/ing/info`)).data,
    retry: false,
    refetchOnWindowFocus: false,
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      /* edit */
      method = 'put';
      url = `${server}iniciativa/${entityId}/draft`;
