...
export default function TimelineTab({ entityId }) {
  ...
  //get ppea_data
  const { data } = useQuery(['iniciative_timeline', { entityId }], {
    queryFn: async () => (await axios.get(`${server}iniciativa/${entityId}/draft/timeline`)).data,
    retry: false,
  ...
  const mutationRemove = useMutation(
    id => {
      return axios.delete(`${server}iniciativa/${entityId}/draft/timeline/${id}`);
    },
    {
      onSuccess: () => queryClient.invalidateQueries(`iniciative_timeline`),
    },
  );
...
function TimelineManager({ add, entityId, editing, onHighlight, onCancel, onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if(add) { /* insert */
        method = 'post';
        url = `${server}iniciativa/${entityId}/draft/timeline`;
      } else {
       /* edit */
        method = 'put';
        url = `${server}iniciativa/${entityId}/draft/timeline/${editing.id}`;
      }
      ...
      queryClient.invalidateQueries('iniciative_timeline');
