...
const Members = () => {
    ...
    const { data } = useQuery(
        ['members_list', { community: currentCommunity.id, page, order, direction, perPage }],
        {
            queryFn: async () => (await axios.get(`${server}gt/${currentCommunity.id}/membership/?page=${page}&order=${order}&direction=${direction === 'asc' ? 'desc' : 'asc'}&limit=${perPage}`)).data,
        }
    );

    const mutations = {
        remove: useMutation((entity) => {
            return axios.delete(`${server}gt/${currentCommunity.id}/membership/${entity.id}`);
    ...
    return (
        <div className="page-content">
            <div className="page-body">
                ...
                <ConfirmationDialog
                    open={!!toRemove}
                    content="Confirma a remoção deste membro?"
