...
import Clipboard from '../../components/icons/clipboard.svg?react';
...
const Invites = () => {
  ...
  const { data } = useQuery(['invites_list', { community: currentCommunity.id, page, order, direction, perPage }], {
    queryFn: async () =>
      (
        await axios.get(
          `${server}gt/${currentCommunity.id}/invites/?page=${page}&order=${order}&direction=${
  ...
  const handleCopyInviteToClipboard = data => async () => {
    try {
      await copyToClipboard(data.link);

      enqueueSnackbar(`Link de convite para ${data.name} foi copiado para a área de transferência!`, {
        variant: 'success',
        anchorOrigin: {
          vertical: 'top',
          horizontal: 'center',
                    ...
                    {data.entities.map(row => (
                      <StyledTableRow key={row.id}>
                        ...
                        <td className="tbox-table-actions">
                          <div>
                            <Tooltip title="Copiar link de convite para a área de transferência">
                              <IconButton onClick={handleCopyInviteToClipboard(row)}>
                                <Clipboard />
                              </IconButton>
                            </Tooltip>
                          </div>
