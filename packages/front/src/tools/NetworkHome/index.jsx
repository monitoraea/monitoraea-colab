...
export default function NetworkHomeCIEA() {
  ...
  const [iniciatives, _iniciatives] = useState(null);
  ...
  const { data } = useQuery(
    ...
    {
      queryFn: async () => (await axios.get(`${server}project/mine?direction=${direction}`)).data,
    }
  );
  ...
  useEffect(() => {
    ...
    // filters
    _iniciatives(prepared);
  }, [data, searchFieldFilter])

  const mutation = {
    create: useMutation(name => axios.post(`${server}project`, { nome: name }), {
        onSuccess: () => {
  ...
  const doParticipate = async (isADM, entityId) => {
    ...
    const { data } = await axios.post(
      `${server}project/${entityId}/participate`,
      {
        isADM,
      },
    );
    ...
    <>
      ...
      <div className="page width-limiter tbox-fixed">
        <div className="page-header">
          <PageTitle title={'Iniciativas (' + total + ')'} />
              ...
              <div className="tbox-body">
                <table className="tbox-table">
                  ...
                  <tbody>
                    ...
                    {iniciatives && iniciatives.map(row => (
